...
class TrustedLauncherProfileTest(unittest.TestCase):
    ...
    def test_td_p1_encodes_three_children_and_nine_exact_replays(self) -> None:
        ...
        self.assertEqual([len(child["cases"]) for child in children], [1, 3, 6])
        self.assertEqual(
            [
                {
                    "id": child["id"],
                    "kind": child["kind"],
                    "python_paths": child["python_paths"],
                    "script": child["script"],
                }
            ...
            [
                {
                    ...
                    "script": {
                        "path": "tools/validate_candidate.py",
                        "root": "subject",
                    },
                },
                {
                    "id": "plebian-hardware-replay",
                    "kind": "python-script",
                    "python_paths": [
                        {
                            "path": "contracts/p1-candidate/tools",
                            "root": "subject",
                        }
                    ],
                    "script": {
        ...
        validator_cases = children[0]["cases"]
        hardware_cases = children[1]["cases"]
        sizer_cases = children[2]["cases"]
        validator_argv = validator_cases[0]["argv"]
        self.assertEqual(
            validator_argv[
        ...
        self.assertEqual(
            [case["argv"] for case in hardware_cases],
            [
                ["show"],
                ["inventory", "--json"],
                ["gpu", "--json"],
        ...
        self.assertEqual(
            [case["stdout"]["path"] for case in hardware_cases],
            [
        ...
        self.assertEqual(
            [case["argv"] for case in sizer_cases],
            [
                ["recommend", "tts", "--json"],
        ...
        self.assertEqual(
            [case["stdout"]["path"] for case in sizer_cases],
            [
        ...
        for case in hardware_cases + sizer_cases:
            self.assertEqual(case["expected_exit"], 0)
            self.assertEqual(case["stdout"]["mode"], "file")
            self.assertEqual(case["stdout"]["root"], "subject")
