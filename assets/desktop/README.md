...
The provisioner writes that path into a new target user's
`~/.local/share/kilix/desktop/.state.json` using the common Kilix/Kilix-95
`wall_image` and `wall_mode=stretch` contract. It creates the state atomically
as the target user only when no state file exists; reprovisioning never rewrites
an existing state or wallpaper choice. `plebian-os-update` deploys later asset
