...
# Two remasters of the same commit must produce the same ISO bytes. Wall-clock
# stamps in build-info.env and the ISO filesystem were enough to make consecutive
# runs diverge. Prefer SOURCE_DATE_EPOCH; otherwise the HEAD committer time.
iso_source_date_epoch() {
    if [[ "${SOURCE_DATE_EPOCH:-}" =~ ^[1-9][0-9]{8,11}$ ]]; then
        printf '%s\n' "$SOURCE_DATE_EPOCH"
        return 0
    fi
    git -C "$HERE" log -1 --format=%ct 2>/dev/null
}
ISO_SOURCE_DATE_EPOCH="$(iso_source_date_epoch)"
[ -n "$ISO_SOURCE_DATE_EPOCH" ] \
...
# Resolve the same source identity that the installed machine will fetch.
# Release metadata retains the local tag, whose HEAD binding is checked by
# release_preflight; firstboot uses the exact commit, publishable before tags.
firstboot_source_ref() {
    local key="$1"
    if [ "$key" = PLEBIAN_OS_REF ] && [ "${PLEBIAN_OS_RELEASE_MODE:-0}" = 1 ]; then
        git -C "$HERE" rev-parse HEAD 2>/dev/null \
            || { echo "could not resolve release checkout commit" >&2; return 1; }
    else
        printf '%s\n' "${!key:-}"
    fi
}
...
validate_release_submodule_gitlinks_resolve() {
    ...
    for ref_key in $(compgen -v | grep -E '_REF$' | sort); do
        ...
        ref="$(firstboot_source_ref "$ref_key")" || exit 1
        repo="${!repo_key:-}"
...
resolve_target_layout() {
    ...
    GPU_TERMINAL_SETTINGS_FILE="$GPU_TERMINAL_HOME/settings.conf"
    ...
    for key in PLEBIAN_OS_TARGET_SOURCE_HOME \
        ...
        PLEBIAN_OS_DIR PLEB_DIR KILIX_DIR KILIX95_DIR KILIX_CAP_DIR \
        KILIX_TUI_UTILS_DIR KILIX_LAND_DESKTOP_DIR PLEB_STORAGE_HOME \
        PLEB_CONFIG_HOME PLEB_STATE_HOME PLEB_CACHE_HOME PLEB_SESSION_HOME \
        PLEB_DATA_HOME KILIX_STORAGE_HOME KILIX_CONFIG_HOME \
        KILIX_STATE_DIRECTORY KILIX_CACHE_HOME KILIX_SESSION_HOME \
...
write_build_info() {
    ...
    gpl2_license_sha="$(sha256sum "$GPL2_LICENSE" | awk '{print $1}')"
    {
        ...
        manifest_kv KILIX_DESKTOP_FLAVOR "${KILIX_DESKTOP_FLAVOR:-95}"
        manifest_kv KILIX_RUN_ALIASES "${KILIX_RUN_ALIASES:-1}"
        ...
        manifest_kv KILIX_TUI_UTILS_AUTO_INSTALL "${KILIX_TUI_UTILS_AUTO_INSTALL:-1}"
        manifest_kv KILIX_TUI_UTILS_DIR "${KILIX_TUI_UTILS_DIR:-}"
        manifest_kv KILIX_TUI_UTILS_REPO "${KILIX_TUI_UTILS_REPO:-https://github.com/itsmygithubacct/kilix-tui-utils.git}"
        manifest_kv KILIX_TUI_UTILS_REF "${KILIX_TUI_UTILS_REF:-}"
        manifest_kv KILIX_TUI_UTILS_TRUST_EXISTING_CHECKOUT "${KILIX_TUI_UTILS_TRUST_EXISTING_CHECKOUT:-0}"
        manifest_kv KILIX_TUI_UTILS_ALLOW_MUTABLE_REF "${KILIX_TUI_UTILS_ALLOW_MUTABLE_REF:-0}"
        manifest_kv KILIX_LAND_DESKTOP_AUTO_INSTALL "${KILIX_LAND_DESKTOP_AUTO_INSTALL:-1}"
        manifest_kv KILIX_LAND_DESKTOP_DIR "${KILIX_LAND_DESKTOP_DIR:-}"
        manifest_kv KILIX_LAND_DESKTOP_REPO "${KILIX_LAND_DESKTOP_REPO:-https://github.com/itsmygithubacct/kilix-land-desktop.git}"
        manifest_kv KILIX_LAND_DESKTOP_REF "${KILIX_LAND_DESKTOP_REF:-}"
        manifest_kv KILIX_LAND_DESKTOP_TRUST_EXISTING_CHECKOUT "${KILIX_LAND_DESKTOP_TRUST_EXISTING_CHECKOUT:-0}"
        manifest_kv KILIX_LAND_DESKTOP_ALLOW_MUTABLE_REF "${KILIX_LAND_DESKTOP_ALLOW_MUTABLE_REF:-0}"
        manifest_kv KILIX_LAND_DESKTOP_ASSETS "${KILIX_LAND_DESKTOP_ASSETS:-}"
        manifest_kv KILIX_LAND_DESKTOP_CONFIG_HOME "${KILIX_LAND_DESKTOP_CONFIG_HOME:-}"
        manifest_kv KILIX_LAND_DESKTOP_EXTERNAL_APPS "${KILIX_LAND_DESKTOP_EXTERNAL_APPS:-}"
        manifest_kv KILIX_LAND_DESKTOP_AUDIO "${KILIX_LAND_DESKTOP_AUDIO:-}"
...
write_firstboot_env() {
    local runtime_os_ref
    runtime_os_ref="$(firstboot_source_ref PLEBIAN_OS_REF)" || exit 1
    {
        ...
        env_kv KILIX_DESKTOP_FLAVOR "${KILIX_DESKTOP_FLAVOR:-95}"
        env_kv KILIX_RUN_ALIASES "${KILIX_RUN_ALIASES:-1}"
        ...
        env_kv KILIX_TUI_UTILS_AUTO_INSTALL "${KILIX_TUI_UTILS_AUTO_INSTALL:-1}"
        env_kv KILIX_TUI_UTILS_DIR "${KILIX_TUI_UTILS_DIR:-}"
        env_kv KILIX_TUI_UTILS_REPO "${KILIX_TUI_UTILS_REPO:-https://github.com/itsmygithubacct/kilix-tui-utils.git}"
        env_kv KILIX_TUI_UTILS_REF "${KILIX_TUI_UTILS_REF:-}"
        env_kv KILIX_TUI_UTILS_TRUST_EXISTING_CHECKOUT "${KILIX_TUI_UTILS_TRUST_EXISTING_CHECKOUT:-0}"
        env_kv KILIX_TUI_UTILS_ALLOW_MUTABLE_REF "${KILIX_TUI_UTILS_ALLOW_MUTABLE_REF:-0}"
        env_kv KILIX_LAND_DESKTOP_AUTO_INSTALL "${KILIX_LAND_DESKTOP_AUTO_INSTALL:-1}"
        env_kv KILIX_LAND_DESKTOP_DIR "${KILIX_LAND_DESKTOP_DIR:-}"
        env_kv KILIX_LAND_DESKTOP_REPO "${KILIX_LAND_DESKTOP_REPO:-https://github.com/itsmygithubacct/kilix-land-desktop.git}"
        env_kv KILIX_LAND_DESKTOP_REF "${KILIX_LAND_DESKTOP_REF:-}"
        env_kv KILIX_LAND_DESKTOP_TRUST_EXISTING_CHECKOUT "${KILIX_LAND_DESKTOP_TRUST_EXISTING_CHECKOUT:-0}"
        env_kv KILIX_LAND_DESKTOP_ALLOW_MUTABLE_REF "${KILIX_LAND_DESKTOP_ALLOW_MUTABLE_REF:-0}"
        env_kv KILIX_LAND_DESKTOP_ASSETS "${KILIX_LAND_DESKTOP_ASSETS:-}"
        env_kv KILIX_LAND_DESKTOP_CONFIG_HOME "${KILIX_LAND_DESKTOP_CONFIG_HOME:-}"
        env_kv KILIX_LAND_DESKTOP_EXTERNAL_APPS "${KILIX_LAND_DESKTOP_EXTERNAL_APPS:-}"
        env_kv KILIX_LAND_DESKTOP_AUDIO "${KILIX_LAND_DESKTOP_AUDIO:-}"
