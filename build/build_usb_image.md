...
answers ─▶ custom preseed ─▶ ISO (remaster-iso.sh) ─▶ dd ─▶ USB stick
                                                             │
                boot a machine from it ─▶ pick install ─▶ unattended install
...
- **xorriso** — builds the ISO (also needs `curl` + `sha256sum` the first time, to
  download + verify the Debian netinst; it's then cached). Not needed with `--iso`.
...
pause** — a deliberate gate on physical hardware. Boot the target machine from the
stick, **pick the install entry**, and the install then runs **completely
unattended** (the preseed answers every question) and first-boot-provisions pleb +
kilix. The pause simply means nothing is erased until *you* select install.

### `--autoboot` (opt-in, hands-off)

`--autoboot` builds a stick whose boot menu **auto-selects the install after a
timeout** — for a kiosk / hands-off deployment.

> **WARNING:** booting any machine from an `--autoboot` stick **auto-installs and
> ERASES that machine's disk with no prompt.** Only build one when that is exactly
> what you want, and don't leave it plugged into a machine that boots from USB.
...
- It **unmounts** every partition on the device before writing, then
