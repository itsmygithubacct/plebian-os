...
class DependencyManifestTests(unittest.TestCase):
    ...
    def test_release_floors_are_enforced_by_a_real_install_run(self):
        ...
        # script: a floor added to a default group later must not turn this
        # test's "at" and "above" cases into a missing-package failure.
        other_floors = {}
        for group in re.findall(r'^\s*"[^"|]+\|([^"]*)"', deps.read_text(), re.M):
            for token in group.split():
                name, sep, version = token.partition("=")
                if sep and name not in ("bubblewrap", "libseccomp2"):
                    other_floors[name] = version
        self.assertIn("libonnxruntime1.21", other_floors)
        for label, versions, expected in cases:
            versions = {**other_floors, **versions}
