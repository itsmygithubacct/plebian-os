...
- Kilix 0.2.2 offers hardware-aware text-to-speech auditions. Piper Kristin
  ...
  fixed synthetic reference, with separate first-use weights. Pocket/Alba is
  now a measured, selectable CPU audition with a locked lazy Python runtime
  and licence-gated first-use weights. Its separate native runner remains
  research-only after malformed near-silent output. None of these changes qualify the release
...
- Kilix Start and status widgets open anchored drop-downs above the panes.
  ...
  `5fe9e45` and `806ea6c` / `d69096c`): drive Kilix panes and tabs from plain
...
- `KILIX_VOICE_REF` advances through `06d1f672` (a selected-model test fix) to
  `c67c5337`, adding interactive TTS tiers, first-use Piper/Qwen/Pocket
  selection, GPU fit visibility and the guarded 0.6B Base audition.
...
- Stage a root-owned, hash-pinned Waydroid first-use closure without putting
  Weston, Android packages, or the roughly 1 GB image set in the base install.
